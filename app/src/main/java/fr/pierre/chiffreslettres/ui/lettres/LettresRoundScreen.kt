...
fun LettresRoundScreen(
    ...
    scoreCumule: Int?,
    onMancheTerminee: (score: Int, motValide: String?, meilleurMot: String?, dixMeilleursMots: List<String>, longueurMotInvalide: Int?, motInvalide: String?) -> Unit,
    ...
    LaunchedEffect(etat.termine) {
        if (etat.termine) {
            ...
            val motInvalide = if (etat.motJoueurValide == false) etat.motSaisi.takeIf { it.isNotEmpty() } else null
            onMancheTerminee(etat.scoreObtenu ?: 0, motValide, etat.meilleurMot, etat.dixMeilleursMots, longueurMotInvalide, motInvalide)
        }
    }
